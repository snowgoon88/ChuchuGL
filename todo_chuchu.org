...
** billboard ??
** line width
** DONE [3/3] dans 3d/texture_screen.hpp and opengltuto/Tron_tuto
<<line_boxing>>
*** DONE display texture
*** DONE [5/5] use framebuffer and Tron texture, as in example : understand Glow
- d'abord, écrit dans un Framebuffer (ParticuleSureface.FboHandle) les boxframe autour des lignes. Ensuite, il "convolue" avec un gradient Glowing. Je crois.
=> Un FrameBuffer avec une texture. On y dessine les boites autour des lignes et ensuite, dans un Shader de Overlay, on convolue cette info de distance avec la texture de Glow. C'est parti.
**** DONE remake part of glow that use geometry shader to draw box around line
**** DONE display tron.png in a top-right overlay
**** DONE render colored lines in Framebuffer and display in top-right
**** DONE draw the texture as planned by Glow
**** DONE "convolution" with the texture
=> d'abord afficher le TronProgram avec une ellipse. 
*** DONE geometry_shader for line boxing
...
**** DONE as Icosahedron WireFrame
**** DONE as Icosahedron Faces
Projection
http://www.rwgrayprojects.com/rbfnotes/maps/graymap2.html
http://www.rwgrayprojects.com/rbfnotes/maps/graymapa.html
**** DONE as Icosaheadron with texture
**** TODO Texture oriented in the same reference frame (or change model)
**** TODO Icosphere vs Icosahedron (I have spherical maps) : divide
**** TODO add reference frame
**** TODO with normals
**** with own trackball ? => move Z vs scale ?
**** with (astro)-physique engine ?
*** [5/12] Better Engine
